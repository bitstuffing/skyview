...
export const GenericSatellite = forwardRef<SatelliteRef, SatelliteProps>(({ 
  scene, 
  earthRadius, 
  noradId,
  color,
  ...
  useEffect(() => {
    ...
    const satelliteMaterial = new THREE.MeshBasicMaterial({ color: new THREE.Color(color) });
    ...
    satelliteRef.current = satelliteMesh;
  ...
  }, [scene, earthRadius, color, noradId]);
  ...
  const updatePosition = () => {
    if (!satrecRef.current || !satelliteRef.current) return;
    ...
    orbitPointsRef.current.push(satelliteRef.current.position.clone());
  ...
  const setVisible = (visible: boolean) => {
    if (satelliteRef.current) satelliteRef.current.visible = visible;
...
export default GenericSatellite;